...
var
  WebModuleClass: TComponentClass = TProgram;
...
procedure TProgram.WebModuleCreate(Sender: TObject);
begin
  FMVC := TMVCEngine.Create(Self,
    ...
    FMVC.AddController(TPersonController);

    var swaggerInfo: TMVCSwaggerInfo;
    FMVC.AddMiddleware(TMVCSwaggerMiddleware.Create(FMVC,swaggerInfo,'/api/swagger.json','Person api'));
    FMVC.AddMiddleware(TMVCStaticFilesMiddleware.Create('/api/swagger','swagger','index.html'));
    FMVC.AddMiddleware(TMVCStaticFilesMiddleware.Create('/index','.','index.html'));
//    FMVC.AddMiddleware(TMVCStaticFilesMiddleware.Create('/users','.','list-person.html'));

    FMVC.AddMiddleware(TMVCCORSMiddleware.Create());


    FMVC.AddMiddleware(TMVCActiveRecordMiddleware.Create(
        dotEnv.Env('TestConnectionDevEnv', 'MyConnDef'),
        dotEnv.Env('TestConnectionConfigDevEnv', 'appstting.ini')
    ));

end;

procedure TProgram.WebModuleDestroy(Sender: TObject);
begin
  FMVC.Free;
end;
